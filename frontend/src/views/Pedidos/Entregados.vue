<template>
  <div>
    <v-data-table 
      :items="pedidos"
      :headers="tbl_headers"
      no-data-text="No hay pedidos entregados para mostrar."
    >
      <template #top>
        <v-toolbar flat>
          <v-toolbar-title>Pedidos Entregados</v-toolbar-title>
          <v-spacer />
...
onMounted(async () => {
  ...
  try {
    const data = await safeFetchJson(apiBase + '/pedidos/entregados')
