<template>
  <div>
    <v-data-table :items="pedidos" :headers="tbl_headers" no-data-text="No hay pedidos listos para entregar que mostrar.">
      <template #top>
        <v-toolbar flat>
            <v-toolbar-title>Pedidos listos para entregar</v-toolbar-title>
            <v-spacer />
...
onMounted(async ()=> {
  const apiBase = getApiBase()
    ...
    try {
      const data = await safeFetchJson(apiBase + '/pedidos/listos-para-entregar')
